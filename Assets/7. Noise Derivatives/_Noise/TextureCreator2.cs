...
public class TextureCreator2 : MonoBehaviour {
	...
	public NoiseDerivativesMethodType type;
	...
	public void FillTexture () {
		...
		NoiseDerivativesMethod method = NoiseDerivatives.noiseMethods[(int)type][dimensions - 1];
		...
		for (int y = 0; y < resolution; y++) {
			...
			for (int x = 0; x < resolution; x++) {
				...
				float sample = NoiseDerivatives.Sum(method, point, frequency, octaves, lacunarity, persistence);
				if (type != NoiseDerivativesMethodType.Value) 
